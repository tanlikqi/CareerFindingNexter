...
export default function useSpecializationSetupService() {
  ...
  const retrieveAllState = async () => {
    ...
    setSpecializationData(res.data as any);
  };
  ...
  const handleOk: SubmitHandler<any> = async (formValues: any) => {
    ...
    try {
      if (!isEdit) {
        const res = await createSpecialization(formValues);
        ...
        const res = await updateSpecialization(formValues);
        if (res.data.resultCode == 1) {
  ...
  const removeState = async (id: any) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await deleteSpecialization({ id: id });
