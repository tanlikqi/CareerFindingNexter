...
export default function useExperienceSetupSerivce() {
      ...
      },
      {
        ...
        getActions: (params: any) => [
          ...
          <GridActionsCellItem
            ...
            label="Delete"
            onClick={() => {
              removeExperience(params.row.id);
            }}
          />,
        ],
      },
  ...
  useEffect(() => {
    retrieveAllExpereince();
  }, []);

  const retrieveAllExpereince = async () => {
    const res = await getAllExperience();
    setExperienceData(res.data as any);
  };
  ...
  const handleOk: SubmitHandler<any> = async (formValues: any) => {
    try {
      if (!isEdit) {
        const res = await createExperience(formValues);
        ...
        const res = await updateExperience(formValues);
        if (res.data.resultCode == 1) {
  ...
  const removeExperience = async (id: any) => {
    console.log(id);
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await deleteExperience({ id: id });
